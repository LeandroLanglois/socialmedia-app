...
export default class UserService {
  ...
  static async createUser(name, email, hashedPassword) {
    await db.query("INSERT INTO register_db (name, email, password) VALUES ($1, $2, $3)", [name, email, hashedPassword])
  }

  static async createPost({ title, description }) {
    const result = await db.query("INSERT INTO posts (title, description) VALUES ($1, $2) RETURNING *", [
      title,
      description,
    ])
  ...
  static async removeLikeDislike(userId, postId) {
    const result = await db.query(`DELETE FROM likes WHERE user_id = $1 AND post_id = $2 RETURNING *`, [userId, postId])
    return result.rows[0]
  }
  ...
  static async getUserReactions(userId) {
    const result = await db.query(`SELECT post_id, value FROM likes WHERE user_id = $1`, [userId])
    return result.rows
  }
}
